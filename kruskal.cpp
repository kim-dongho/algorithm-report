...
void union_t(int a,int b) // 정점 a와 정점 b의 합병을위한 함수 union_t 
{
	if(arr[a] > arr[b]){ // arr[a]의 값이 작을 경우
		arr[b] += arr[a]; // arr[b]의 값을 arr[a]에 저장 
		arr[a] = b; // arr[b]의 값은 그 값의 부모노드인 a의 값이 된다. 
	}
	else{ // 두 값이 같거나 arr[a]가 더 클때 
		arr[a] += arr[b]; // arr[a]의 값을 arr[b]에 저장 
		arr[b] = a; // arr[a]의 값은 그 값의 부모노드인 b값이 된다. 
	}
}
int main()
{
	vector<vector <int> > v; // 정점 두개와 비용값을 저장하기 위한 int형 이차원 벡터 v 
